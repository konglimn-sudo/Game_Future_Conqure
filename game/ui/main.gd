...
var assign_mode := false   # 行军指派：等待点击目标控制区
var milestones_done := {}
...
func _ready() -> void:
	...
	_log("⚔ 战争（M2）：选敌方区域「宣战」→ 选驻军区「进攻」点相邻敌区；中立国可直接武力吞并（民兵抵抗，恶名 +1 计入反霸权）。「指派行军」可跨多省自动行军到边境。")
...
func _unhandled_input(ev: InputEvent) -> void:
	if ev is InputEventKey and ev.pressed and not ev.echo:
		match ev.keycode:
			...
			KEY_ESCAPE:
				if move_mode or attack_mode or assign_mode:
					move_mode = false
					attack_mode = false
					assign_mode = false
					_log("已取消指令")
		...
		for ring in rings[id]:
			if Geometry2D.is_point_in_polygon(pos, ring):
				if move_mode and selected >= 0:
					...
					_refresh()
					return
				if assign_mode and selected >= 0:
					assign_mode = false
					var why3 := sim.can_set_move_order(selected, id)
					if why3 == "":
						sim.set_move_order(selected, id)
						_flush_events()
					else:
						_log("🛡 无法指派：" + why3)
					_refresh()
...
func _on_action(kind: String) -> void:
	...
	match kind:
		...
		"army_assign":
			assign_mode = true
			_log("🛡 指派模式：点击任意己方控制区（如边境省），部队将每月自动行军（Esc 取消）")
		"war":
...
func _refresh_region() -> void:
	...
	if sim.owner_of(r) == -1:
		t += "民兵抵抗：🪖%d（武力吞并需击溃，且恶名 +1）\n" % sim.militia_of(r)
	var ot := sim.order_target(selected)
	if ot >= 0:
		t += "行军目标：〔%s〕\n" % sim.region(ot)["name"]
